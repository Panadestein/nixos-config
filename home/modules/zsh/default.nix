...
{ config, pkgs, ... }:
{
  programs.zsh = {
    ...
    initContent = ''
      ...
      ldiff() {
          if (( $# != 2 )); then
              print -u2 'Usage: ldiff FILE FILE'
              return 1
          fi
          # Process substitution avoids overwriting /tmp files with shared basenames.
          nvim -R -d =(cat -n -- "$1") =(cat -n -- "$2")
      }
      ...
      recscr() {
          # Record the Wayland display; stop recording with Ctrl+C.
          if (( $# != 1 )); then
              print -u2 'Usage: recscr FILENAME.mp4'
              return 1
          fi
          mkdir -p "$HOME/Videos" || return
          wf-recorder -f "$HOME/Videos/$1"
      }
